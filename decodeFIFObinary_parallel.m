...
macroTOffset=0;
photonCount=0;
numPhotIni=numel(find(bitand(fblock,INVALID32)==0)); %Number of valid events 
data_MacroMicroTime=zeros(numPhotIni, 2, 'double');
data_channel=zeros(numPhotIni,1,'uint8');
lineEventcounter=0;
...
nr_RoutChannels=bitshift(bitand(event1,RB_NO32),-27)+1; %Nr. of routing channels
...
switch acqType
    case {1,2} %FIFO Image or Scanning FCS
        data_frameLinePixel=zeros(numPhotIni, 3, 'uint32');
...
spmd (numWorkers)
    ...
    parNumPhot=numel(find(bitand(parFrame,INVALID32)==0)); %Nr. of photons per worker
    if acqType<3, %not FIFO Point
        parData_frameLinePixel=zeros(parNumPhot,3,'uint32');
    end
    parData_MacroMicroTime=zeros(parNumPhot,2,'double'); 
    parData_channel=zeros(parNumPhot,1,'uint8');        
...
numPhotAfterPar=sum(matPhotonCount); % Number of photons after parallelisation
data_MacroMicroTime(numPhotAfterPar+1:end,:)=[]; 
data_channel(numPhotAfterPar+1:end,:)=[]; 

switch acqType
    case {1,2}
        data_frameLinePixel(numPhotAfterPar+1:end,:)=[];
        ...
        numPhotFin= size(data_channel,1);% Number of photons for the whole acquisition
        ...
        if acqType==1 %FIFO Image
            for c=1:numPhotFin %Image recontruction (FIFO Image)
                twoDIntensity(data_frameLinePixel(c,2),data_frameLinePixel(c,3),data_channel(c,1)+1)=twoDIntensity(data_frameLinePixel(c,2),data_frameLinePixel(c,3),data_channel(c,1)+1)+1;
            end     %end for(c)
            ...
            for f=1:numPhotFin % Image recontruction (Scanning FCS)
                twoDIntensity(rSum(f)+data_frameLinePixel(f,2),data_frameLinePixel(f,3),data_channel(f,1)+1)=...
                    twoDIntensity(rSum(f)+data_frameLinePixel(f,2),data_frameLinePixel(f,3),data_channel(f,1)+1)+1;
            end     % end for(f)
        end %end if (FIFO Image/Scanning FCS)
